...
export default {
  ...
  methods: {
    getList() {
      this.$store.dispatch('ingredients/getInfoExportHouseware', this.id).then(res => {
    ...
    chooseOption(index, indexOption) {
      this.options[indexOption].ingredients_id = this.housewareOption[index].id;
    ...
    update() {
      ...
      this.$store.dispatch('ingredients/updateExportHouseware', params).then(() => {
